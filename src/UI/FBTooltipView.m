...
CGMutablePathRef _createUpPointingBubbleWithRect(CGRect rect, CGFloat arrowMidpoint, CGFloat arrowHeight, CGFloat radius);
CGMutablePathRef _createDownPointingBubbleWithRect(CGRect rect, CGFloat arrowMidpoint, CGFloat arrowHeight, CGFloat radius);

#pragma mark -

@implementation FBTooltipView
{
    ...
    FBTooltipColorStyle _colorStyle;
...
- (void)dealloc
{
    ...
    [_crossCloseGlyphColor release];
    [_textLabel release];
    [_innerStrokeColor release];
    [_insideTapGestureRecognizer removeTarget:self action:NULL];
    [_insideTapGestureRecognizer release];
    [_message release];
    [_tagline release];
...
- (FBTooltipColorStyle)colorStyle
{
    return _colorStyle;
}

- (void)setColorStyle:(FBTooltipColorStyle)colorStyle
{
    ...
    switch (colorStyle) {
        case FBTooltipColorStyleNeutralGray:
            _gradientColors = [@[
                                 (id)(FBUIColorWithRGB(0x51, 0x50, 0x4f).CGColor),
                                 (id)(FBUIColorWithRGB(0x2d, 0x2c, 0x2c).CGColor)
                                 ] retain];
            _innerStrokeColor = [[UIColor colorWithWhite:0.13f alpha:1.0f] retain];
            _crossCloseGlyphColor = [[UIColor colorWithWhite:0.69f alpha:1.0f] retain];
            break;

        case FBTooltipColorStyleFriendlyBlue:
        default:
            _gradientColors = [@[
                                 (id)(FBUIColorWithRGB(0x6e, 0x9c, 0xf5).CGColor),
                                 (id)(FBUIColorWithRGB(0x49, 0x74, 0xc6).CGColor)
                                 ] retain];
            _innerStrokeColor = [[UIColor colorWithRed:0.12f green:0.26f blue:0.55f alpha:1.0f] retain];
            _crossCloseGlyphColor = [[UIColor colorWithRed:0.60f green:0.73f blue:1.0f alpha:1.0f] retain];
            break;
    }
...
- (void)animateFadeIn
{
    ...
    self.layer.transform = fbdfl_CATransform3DConcat(fbdfl_CATransform3DMakeScale(kZoomOutScale, kZoomOutScale, kZoomOutScale),
    ...
    void (^zoomIn)() = ^() {
        ...
        CATransform3D scale = fbdfl_CATransform3DMakeScale(kZoomInScale, kZoomInScale, kZoomInScale);
        CATransform3D translate =fbdfl_CATransform3DMakeTranslation(newZoomOffsetX, newZoomOffsetY, 0);
        self.layer.transform = fbdfl_CATransform3DConcat(scale, translate);
    };
    ...
    // 3rd Step.
    void (^normalizeZoom)() = ^() {
        self.layer.transform = fbdfl_CATransform3DIdentity;
    };
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (_pointingUp) {
        outerPath = _createUpPointingBubbleWithRect(outerRect,
                                                    _arrowMidpoint, _arrowHeight,
                                                    kNUXCornerRadius + kNUXStrokeLineWidth);
        innerPath = _createUpPointingBubbleWithRect(innerRect,
                                                    _arrowMidpoint, _arrowHeight,
                                                    kNUXCornerRadius);
        fillPath = _createUpPointingBubbleWithRect(fillRect,
                                                   _arrowMidpoint, _arrowHeight,
                                                   kNUXCornerRadius - kNUXStrokeLineWidth);
        gradientRect.origin.y -= _arrowHeight;
        gradientRect.size.height += _arrowHeight;
    } else {
        outerPath = _createDownPointingBubbleWithRect(outerRect,
                                                      _arrowMidpoint, _arrowHeight,
                                                      kNUXCornerRadius + kNUXStrokeLineWidth);
        innerPath = _createDownPointingBubbleWithRect(innerRect,
                                                      _arrowMidpoint, _arrowHeight,
                                                      kNUXCornerRadius);
        fillPath = _createDownPointingBubbleWithRect(fillRect,
                                                     _arrowMidpoint, _arrowHeight,
                                                     kNUXCornerRadius - kNUXStrokeLineWidth);
        gradientRect.size.height += _arrowHeight;
    }
...
- (void)setMessage:(NSString *)message tagline:(NSString *)tagline
{
    ...
    if ([_textLabel respondsToSelector:@selector(attributedText)]) {
        ...
        if ([tagline length]) {
            [attrString addAttribute:NSForegroundColorAttributeName value: FBUIColorWithRGB(0x6D, 0x87, 0xC7) range:NSMakeRange(0, [tagline length])];
        }
